...
class HrContractSetting(models.Model):
    ...
    contract_id = fields.Many2one('hr.contract',
                                  ...
                                  ondelete='cascade',
                                  select=True)
    contrib_id = fields.Many2one('hr.contribution.register',
                                 'Concepto',
                                 help="Concepto de aporte")
    partner_id = fields.Many2one('res.partner',
                                 'Entidad',
                                 help="Entidad relacionada")
...
class HrContractChangeWage(models.Model):
    ...
    wage = fields.Float('Salario basico',
                        help="Seguimento de los cambios en el salario ´basico",
                        required=True)
    contract_id = fields.Many2one('hr.contract',
                                  'Riesgos',
                                  required=True,
...
class HrContractDeduction(models.Model):
    ...
    input_id = fields.Many2one('hr.rule.input',
                               'Regla salarial',
                               required=True,
                               help="Parámetro asociada a la regla salarial")
    period = fields.Selection([('limited', 'Limitado'),
    ...
    amount = fields.Float(
        ...
        default=0,
        help=
        "Valor de la cuota o porcentaje segun formula de la regla salarial",
        required=True)
    total_deduction = fields.Float('Total obligación',
                                   default=0,
    ...
    total_acumulados = fields.Float('Acumulado Odoo',
...
class HrContract(models.Model):
    ...
    analytic_ids = fields.One2many('hr.contract.analytic', 'contract_id',
                                   'Cuentas analíticas')
    setting_ids = fields.One2many('hr.contract.setting', 'contract_id',
                                  'Configuración')
    deduction_ids = fields.One2many('hr.contract.deduction', 'contract_id',
                                    'Deducciones')
    liquidacion_ids = fields.One2many('hr.contract.liquidacion', 'contract_id',
                                      'Liquidación')
    acumulado_ids = fields.One2many('hr.contract.acumulados', 'contract_id',
                                    'Acumulados')
    change_wage_ids = fields.One2many('hr.contract.change.wage', 'contract_id',
                                      'Cambios salario')
    risk_id = fields.Many2one('hr.contract.risk',
                              required=True,
                              string='Riesgo profesional')
    ...
    distribuir = fields.Boolean(
        'Distribuir por cuenta analítica',
        default=False,
        help=
        "Indica si al calcula la nómina del contrato se distribuye por centro de costo"
    )
    ...
    compensacion = fields.Float('Compensación',
                                default=0.0,
                                digits_compute=dp.get_precision('Payroll'))
