	...
	def get_inputs(self, contract_ids, date_from, date_to):
		...
		for contract in contracts:
			for input in inputs:
				...
				amount = 0.00
				param = [('contract_id', '=', contract.id), ('input_id', '=', input.id)]
					...
					for deduction in deduction_ids:
						...
						if deduction.period == 'limited':
							if deduction.total_deduction > deduction.total_accumulated:
								if (deduction.total_accumulated + deduction.amount) > deduction.total_deduction:
	...
	def action_payslip_done(self):
		...
		for slip in self:
			...
			for line in slip.details_by_salary_rule_category:
				...
				if slip.contract_id:
					registers = self.env['hr.contract.register'].browse(slip.contract_id.register_ids)
