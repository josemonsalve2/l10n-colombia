...
class HrContractDeduction(models.Model):
    ...
    contract_id = fields.Many2one(string='Deductions',
    ...
    def _compute_total_accumulated(self):
        deduction_line_ids = self.env['hr.payslip.deduction.line'].search([
            ('deduction_id', '=', self.id), ('slip_id.state', '=', 'done')
        ])
        deduction_lines = self.env['hr.payslip.deduction.line'].browse(
            deduction_line_ids)
