    ...
    def compute_sheet(self):
        ...
        if active_id:
            [run_data] = self.env['hr.payslip.run'].browse(active_id).read([
                ...
                'journal_id', 'liquida', 'tipo_liquida', 'struct_liquida_id',
                'date_liquidacion', 'date_prima', 'date_cesantias',
                'date_vacaciones'
            ])
        ...
        #++
        if run_data.get('liquida',
                        False) and not run_data.get('struct_id', False):
            raise UserError(
                _("Debe seleccionar una estrutura salarial para liquidar"))
        ...
        for employee in self.env['hr.employee'].browse(data['employee_ids']):
            ...
            res = {
                ...
                to_date,
                'credit_note':
                run_data.get('credit_note'),
                #++
                'liquida':
                run_data.get('liquida', False),
                'tipo_liquida':
                run_data.get('tipo_liquida', False),
                'motivo_retiro':
                run_data.get('motivo_retiro', False),
                'struct_liquida_id':
                liquida_struct_id,
                'date_liquidacion':
                run_data.get('date_liquidacion', False),
                'date_prima':
                run_data.get('date_prima', False),
                'date_cesantias':
                run_data.get('date_cesantias', False),
                'date_vacaciones':
                run_data.get('date_vacaciones', False),
                'journal_id':
                journal_id,
            }
