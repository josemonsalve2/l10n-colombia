...
class AccountInvoiceDebitNote(models.TransientModel):
    ...
    date_invoice = fields.Date(string='Debit Note Date',
    ...
    filter_debit_note = fields.Selection(
        ...
        required=True,
        help=
        'Debit Note base on this type. You can not Modify and Cancel if the invoice is '
        'already reconciled')
        ...
        for form in self:
            ...
            for inv in inv_obj.browse(context.get('active_ids')):
                ...
                refund.update({
                    'refund_type':
                    'debit',
                    'discrepancy_response_code_id':
                    form.discrepancy_response_code_id.id,
                    'name':
                    'debit'
                })
    ...
    def invoice_debit_note(self):
        data_debit_note = self.read(['filter_debit_note'
                                     ])[0]['filter_debit_note']
