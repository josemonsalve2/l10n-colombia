...
class AccountInvoiceLine(models.Model):
    ...
    def recompute_tax_id(self):
        ...
        for line in self:
            ...
            taxes = taxes.filtered(lambda r, company_id=line_company_id: r.
                                   company_id == company_id)
            taxes = fpos.map_tax(taxes, line.product_id,
        ...
        for line in self:
            ...
            incl_tax = taxes.filtered(
                lambda tax: tax not in line.invoice_line_tax_ids and tax.
                price_include)
    ...
    def recompute_fix_price(self, taxes, fp_taxes):
        ...
        if self.invoice_id.type in ('in_invoice', 'in_refund'):
            ...
            if (not self.price_unit
                    or float_compare(self.price_unit,
                                     self.product_id.standard_price,
                                     precision_digits=prec) == 0):
                self.update({
                    'price_unit':
                    fix_price(self.product_id.standard_price, taxes, fp_taxes)
                })
                self._set_currency()
        else:
            self.update({
                'price_unit':
                fix_price(self.product_id.lst_price, taxes, fp_taxes)
            })
            self._set_currency()
