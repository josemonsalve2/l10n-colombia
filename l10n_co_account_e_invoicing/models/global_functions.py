...
def get_cufe_cude(NumFac, FecFac, HorFac, ValFac, CodImp1, ValImp1, CodImp2,
    ...
    uncoded_value = (NumFac + ' + ' + FecFac + ' + ' + HorFac + ' + ' +
                     ...
                     ' + ' + TipoAmbie)
    CUFE_CUDE = hashlib.sha384(NumFac + FecFac + HorFac + ValFac + CodImp1 +
                               ValImp1 + CodImp2 + ValImp2 + CodImp3 +
                               ValImp3 + ValTot + NitOFE + DocAdq +
...
def get_template_xml(values, template_name):
    ...
    env = Environment(
        loader=FileSystemLoader(path.join(base_path, 'templates')))
...
def get_pkcs12(certificate_file, certificate_password):
    ...
    try:
        return crypto.load_pkcs12(b64decode(certificate_file),
                                  certificate_password)
