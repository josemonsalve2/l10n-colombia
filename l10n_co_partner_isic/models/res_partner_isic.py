...
class RespartnerIsic(models.Model):
    ...
    name = fields.Char(string="Name", help="ISIC Name", required=True, index=1)
    display_name = fields.Char(string="Display Name", compute="_compute_display_name")
    code = fields.Char(string="Code", help="ISIC code", required=True, index=1)
    ...
    parent_id = fields.Many2one(
        string="Parent", comodel_name="res.partner.isic", ondelete="cascade"
    )
    child_ids = fields.One2many(
        string="Childs Codes", comodel_name="res.partner.isic", inverse_name="parent_id"
    )
    parent_left = fields.Integer("Parent Left", index=1)
    parent_right = fields.Integer("Parent Right", index=1)
